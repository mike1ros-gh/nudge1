...
export default function DiagnosticsPage() {
  ...
  const redisUsagePercent =
    data?.redisUsage?.limit == null
      ? null
      : Math.round((data.redisUsage.used / data.redisUsage.limit) * 100);
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="panel rounded p-5">
          ...
          <p
            className={`mt-3 text-2xl font-bold ${
              data?.workerHealth.healthy
                ? "text-success"
                : data?.workerHealth.stalled
                  ? "text-error"
                  : "text-warning"
            }`}
          >
          ...
          <p className="mt-2 text-xs text-muted">
            {data?.workerHealth.stalled
              ? `${data.workerHealth.queueWaiting} job${data.workerHealth.queueWaiting === 1 ? "" : "s"} waiting, nothing picked up recently`
        ...
        <div className="panel rounded p-5">
          <p className="text-xs font-semibold uppercase text-muted">
            Redis commands (this month)
          </p>
          {data?.redisUsage ? (
            <>
              <p
                className={`mt-3 text-2xl font-bold ${
                  redisUsagePercent != null && redisUsagePercent >= 90
                    ? "text-error"
                    : redisUsagePercent != null && redisUsagePercent >= 70
                      ? "text-warning"
                      : "text-foreground"
                }`}
              >
                {data.redisUsage.used.toLocaleString()}
                {data.redisUsage.limit != null
                  ? ` / ${data.redisUsage.limit.toLocaleString()}`
                  : ""}
              </p>
              <p className="mt-2 text-xs text-muted">
                {data.redisUsage.databaseName} · {data.redisUsage.plan}
                {redisUsagePercent != null ? ` · ${redisUsagePercent}%` : ""}
              </p>
            </>
          ) : (
        ...
        {(
          [
            { key: "waiting", tone: "default" },
            { key: "active", tone: "sent" },
            { key: "delayed", tone: "skipped" },
            { key: "failed", tone: "failed" },
          ] as const
        ).map(({ key, tone }) => (
          <StatCard
            key={key}
