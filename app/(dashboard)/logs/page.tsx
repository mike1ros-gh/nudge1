...
const STATUS_FILTERS = ["ALL", "SENT", "FAILED", "PENDING", "SKIPPED_RATE_LIMIT"];

const STATUS_DESCRIPTIONS: Record<string, string> = {
  ALL: "Every DM attempt Nudge1 has made, no matter how it turned out.",
  SENT: "The DM went out successfully — nothing more to do here.",
  FAILED:
    "Nudge1 tried and gave up. Either Instagram rejected it (e.g. the commenter doesn't allow DMs from you) or something kept failing after a few automatic retries. It will not try again on its own — click the row for the error.",
  PENDING:
    "Nudge1 is about to send this, or is waiting to retry it after a temporary hiccup. It should move to Sent or Failed within a few minutes.",
  SKIPPED_RATE_LIMIT:
    "Instagram's hourly DM limit was reached, so Nudge1 skipped this one on purpose to keep the account safe. It won't retry automatically — it'll catch up once a new comment comes in and the limit has reset.",
};
...
export default function LogsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="-mt-2 text-xs text-muted">{STATUS_DESCRIPTIONS[statusFilter]}</p>
