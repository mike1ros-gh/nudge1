...
export async function GET(request: NextRequest) {
  ...
  const alertableWorkspaces = await prisma.workspace.findMany({
    where: { alertsEnabled: true },
    select: { id: true },
  });
  for (const workspace of alertableWorkspaces) {
    try {
      await checkAndSendAlerts(workspace.id);
    } catch (err) {
      console.error(`[Cron] Failed to check/send alerts for workspace ${workspace.id}:`, err);
    }
  }
  ...
  return NextResponse.json({
    ...
    data: {
      ...
      alertsChecked: alertableWorkspaces.length,
      results,
    },
  });
}
