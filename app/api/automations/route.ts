...
import { activeKeywordCounts, calculateCtr } from "@/lib/tracking/analytics";
...
const createAutomationSchema = z
  .object({
    ...
    followStepEnabled: z.boolean().optional().default(false),
    followStepMessage: z.string().max(1000).optional().nullable(),
    followStepButtonLabel: z.string().max(64).optional().nullable(),
  ...
  // A follow step needs both a message and a button label...
  .refine(
    (d) =>
      !d.followStepEnabled ||
      (Boolean(d.followStepMessage?.trim()) &&
        Boolean(d.followStepButtonLabel?.trim())),
    { message: "Follow step needs a message and a button label", path: ["followStepMessage"] }
  )
  // ...and can only exist between an opening DM and the reveal.
  .refine((d) => !d.followStepEnabled || d.openingDmEnabled, {
    message: "Follow step requires an opening DM",
    path: ["followStepEnabled"],
  });
...
const updateAutomationSchema = z.object({
  ...
  openingDmMessage: z.string().max(1000).optional().nullable(),
  openingDmButtonLabel: z.string().max(64).optional().nullable(),
  followStepEnabled: z.boolean().optional(),
  followStepMessage: z.string().max(1000).optional().nullable(),
  followStepButtonLabel: z.string().max(64).optional().nullable(),
  linkButtonLabel: z.string().max(20).optional().nullable(),
...
export async function GET(request: NextRequest) {
  ...
  for (const automation of automationsWithReports) {
    ...
    if (!item) continue;
    item.topKeywords = activeKeywordCounts(
      automation.keywords,
...
export async function POST(request: NextRequest) {
  ...
  const automation = await prisma.automation.create({
    data: {
      ...
      followStepMessage:
        openingDmEnabled && followStepEnabled
          ? parsed.data.followStepMessage || null
          : null,
      followStepButtonLabel:
        openingDmEnabled && followStepEnabled
          ? parsed.data.followStepButtonLabel || null
          : null,
...
export async function PATCH(request: NextRequest) {
  ...
  // opening DM clears its message and button (and, since a follow step can
  // only exist between an opening DM and the reveal, its follow step too).
  ...
  if (automationData.openingDmEnabled === false) {
    ...
    automationData.followStepEnabled = false;
    automationData.followStepMessage = null;
    automationData.followStepButtonLabel = null;
  }
  if (automationData.followStepEnabled === false) {
    automationData.followStepMessage = null;
    automationData.followStepButtonLabel = null;
  }
