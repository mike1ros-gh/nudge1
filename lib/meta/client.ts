...
import { DEFAULT_UTC_OFFSET_MINUTES, formatDateKey } from "@/lib/utils/timezone";
...
/**
 * Send a direct message as a button template with a postback button — used
 * for the optional follow-step message between the opening DM and the
 * reveal. Tapping the button fires another `messaging_postbacks` webhook
 * carrying `payload`, same as the opening DM's button.
 */
...
/**
 * Daily new-follower counts from Instagram's own account-level insights
 * (not a running total — each value is that day's gain). Meta documents up
 * to ~48h of lag before a day's value is fully settled, so the caller
 * should treat the most recent 1-2 days as provisional.
 */
export async function getFollowerGrowth(
  ...
  return values.map((v) => ({
    date: formatDateKey(new Date(v.end_time), utcOffsetMinutes),
    gained: v.value,
  }));
}
