 ...
 *
 * Trigger scope (specific / any / next post), match mode (specific words /
 * any word), the opening + follow step + reveal DM text, public reply, and
 * the tracked link are all wired end to end. Email / other follow-up steps
...
export default function CampaignBuilder({ mode, campaignId }: CampaignBuilderProps) {
  ...
  const [followStepEnabled, setFollowStepEnabled] = useState(false);
  const [followStepMessage, setFollowStepMessage] = useState("");
  const [followStepButtonLabel, setFollowStepButtonLabel] = useState("");
  ...
  useEffect(() => {
    ...
    fetch("/api/automations", { cache: "no-store" })
      ...
      .then((payload) => {
        ...
        setFollowStepEnabled(c.followStepEnabled);
        setFollowStepMessage(c.followStepMessage ?? "");
        setFollowStepButtonLabel(c.followStepButtonLabel ?? "");
  ...
  function prefillFromRow(row: ImportRow) {
    ...
    // Not part of CSV import rows — reset so nothing leaks between rows.
    setFollowStepEnabled(false);
    setFollowStepMessage("");
    setFollowStepButtonLabel("");
  ...
  // A follow step only makes sense between an opening DM and the reveal, so
  // turning the opening DM off takes the follow step with it.
  function toggleOpeningDm() {
    setOpeningDmEnabled((cur) => {
      if (cur) setFollowStepEnabled(false);
      return !cur;
    });
  }
  ...
  async function handleSubmit(activeValue: boolean) {
    ...
    if (
      followStepEnabled &&
      (!followStepMessage.trim() || !followStepButtonLabel.trim())
    )
      return setError("Your follow step needs a message and a button label.");
    ...
    const payload = {
      ...
      followStepEnabled: openingDmEnabled && followStepEnabled,
      followStepMessage:
        openingDmEnabled && followStepEnabled ? followStepMessage : null,
      followStepButtonLabel:
        openingDmEnabled && followStepEnabled ? followStepButtonLabel : null,
      publicReplyEnabled,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-8">
        ...
        {openingDmEnabled && (
          <Section title="Then, they will get">
            <div className="rounded-lg border border-border p-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-foreground">a follow reminder</span>
                <Toggle
                  on={followStepEnabled}
                  onToggle={() => setFollowStepEnabled(!followStepEnabled)}
                />
              </div>
              {followStepEnabled && (
                <div className="mt-3 space-y-2">
                  <textarea
                    value={followStepMessage}
                    onChange={(e) => setFollowStepMessage(e.target.value)}
                    placeholder="One more thing — make sure you're following me!"
                    rows={3}
                    className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none resize-none"
                    maxLength={1000}
                  />
                  <input
                    value={followStepButtonLabel}
                    onChange={(e) => setFollowStepButtonLabel(e.target.value)}
                    placeholder="I followed, show me"
                    className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-foreground placeholder:text-zinc-500 focus:border-accent/40 focus:outline-none"
                    maxLength={64}
                  />
                  <p className="text-xs text-muted">
                    Instagram doesn&apos;t let us check who&apos;s actually
                    following — this is an honor-system nudge, not an enforced
                    gate.
                  </p>
                </div>
              )}
      ...
      <div>
        ...
        <div className="flex justify-center lg:sticky lg:top-6 lg:block">
          <CampaignPreview
            ...
            followStepEnabled={followStepEnabled}
            followStepMessage={followStepMessage}
            followStepButtonLabel={followStepButtonLabel}
